...
public class MazewarConsole implements INetworkPeer
{
    ...
    public static void main(String args[])
    {
        //
        // Arguments:
        //  java MazewarConsole <myName> <myHost> <myPort> [<peerName> <peerPort>]
        //
        try
        {
            ...
            if (args.length == 5)
        	{
        		String peerHost = args[3];
        		int peerPort = Integer.parseInt(args[4]);

                server = new MazewarServer(myHost, myPort, console, peerHost, peerPort);
            }
            else
            {
                server = new MazewarServer(myHost, myPort, console);
            }
        }
        catch (Exception e)
        {
