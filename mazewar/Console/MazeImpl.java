...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        private MazeWarServer _Server = null;
        private Thread _ServerThread = null;

        private Integer _CurrentTime = 0;
        ...
        public synchronized Integer getNewEventTimestamp ()
        {
            _CurrentTime++;
        ...
        public void syncEventTimestamp (Integer incomingTimestamp)
        {
            _CurrentTime = Math.max(_CurrentTime, incomingTimestamp);
        }

        public MazeWarServer getServer ()
        {
            return _Server;
        }

        public MazeImpl(Point point, long seed, PeerLocation me)
            throws IOException
        {
                maxX = point.getX();
                assert(maxX > 0);
                ...
                _Server = new MazeWarServer(this, me);
                ...
                throws IOException, ClassNotFoundException
        {
            ...
            // Tell the peers I want to join the game. When this eventually delivers a
            // message back to the maze (with all the peer locations) and the others
            // have already been added, choose a location which isn't yet occupied
            // and put me there.
            PeerInfo myInfo = new PeerInfo();
                     myInfo.EventSource = _Server.getMe();
                     myInfo.RequestType = PeerMessage.INIT_NEW_CLIENT;
                     myInfo.Timestamp = this.getNewEventTimestamp();
                     myInfo.State = new PeerState();
                     myInfo.State.Name = _Server.getMe().Name;
                     myInfo.State.Position = null;
                     myInfo.State.Orientation = null;

            // Bypass all the broadcast stuff if we're the only one.
            if (_Server.getPeers().size() > 1)
            {
                _Server.BroadcastMessage(myInfo);
            }
            else
            {
                _Server.setInitialPeerState(client, myInfo.State);
                _Server.DeliverMessage(myInfo);
            }
        }

        public synchronized void finishAddLocalClient(LocalClient client)
        {
            ...
            addClientAtLocation(client, point, direction);
        }

        public void addRemoteClient(RemoteClient client, Point point, Direction direction)
        {
            addClientAtLocation(client, point, direction);
        }

        public void addClientAtLocation(Client client, Point point, Direction direction) {
                assert(client != null);
        ...
        public synchronized void setClientLocation(Client client, DirectedPoint dp)
        {
            assert(client != null);
            assert(dp != null);
        ...
        public synchronized boolean clientFire(Client client) {
            return clientFire(client, false);
        }

        public boolean canClientFire(Client client) {
            return clientFire(client, true);
        }

        public synchronized boolean clientFire(Client client, boolean idempotent) {
                assert(client != null);
        ...
        public synchronized boolean moveClientForward(Client client) {
                ...
                assert(o instanceof DirectedPoint);
                DirectedPoint dp = (DirectedPoint)o;
                return moveClient(client, dp.getDirection(), false);
        }

        public boolean canMoveClientForward(Client client) {
                assert(client != null);
                Object o = clientMap.get(client);
                assert(o instanceof DirectedPoint);
                DirectedPoint dp = (DirectedPoint)o;
                return moveClient(client, dp.getDirection(), true);
        }
        ...
        public synchronized boolean moveClientBackward(Client client) {
                ...
                assert(o instanceof DirectedPoint);
                DirectedPoint dp = (DirectedPoint)o;
                return moveClient(client, dp.getDirection().invert(), false);
        }

        public boolean canMoveClientBackward(Client client) {
                assert(client != null);
                Object o = clientMap.get(client);
                assert(o instanceof DirectedPoint);
                DirectedPoint dp = (DirectedPoint)o;
                return moveClient(client, dp.getDirection().invert(), true);      
        }
         ...
         */
        private synchronized boolean moveClient(Client client, Direction d, boolean idempotent) {
                assert(client != null);
                assert(d != null);
